...
enum TracingState tracing_state = TRACING;
...
struct Status status64 = {
  .daemon_pid = -1
};
struct Status status32 = {
  .daemon_pid = -1
};
...
void rezygiskd_listener_callback() {
  ...
  while (1) {
    ...
    if ((size_t)nread < sizeof(enum Command)) {
      LOGE("read %zu < %zu", nread, sizeof(enum Command));
      continue;
    }
    ...
    if (msg->cmd >= DAEMON64_SET_INFO && msg->cmd != SYSTEM_SERVER_STARTED) {
      ...
      if (nread != sizeof(enum Command)) {
        LOGE("cmd %d size %zu != %zu", msg->cmd, nread, sizeof(enum Command));

        continue;
      }

      real_size = sizeof(enum Command);
    }
...
static bool ensure_daemon_created(bool is_64bit) {
  struct Status *status = is_64bit ? &status64 : &status32;
  if (is_64bit) {
...
static bool prepare_environment() {
  snprintf(prop_path, sizeof(prop_path), "%s/module.prop", rezygiskd_get_path());
